...
AAL <- tq_get("MSFT", get = "stock.prices", from = "2000-01-01", to = "2020-09-17")
...
```{r}
monthly_returns_stocks <- c("AMZN", "ADBE", "MSFT", "AAL") %>%
...
```{r}
stock_returns_monthly <- c("AMZN", "ADBE", "MSFT","AAL") %>%
tq_get(get  = "stock.prices", from = "2000-01-01",to   = "2020-09-17") %>%
group_by(symbol) %>%
...
#americanairlines
AAL_dates <- select(AAL,date)
AAL_dates

data.frame_AAL <- data.frame(AAL_dates,sma_AAL_200, sma_AAL)
data.frame_AAL
...
data.frame_AAL['AAL'] <- 0
data.frame_AAL$AAL[data.frame_AAL$sma_AAL > data.frame_AAL$sma_AAL_200] = 1  
...
#AAL
V_04 <- select(data.frame_AAL, AAL)
colSums(V_04, na.rm = TRUE)

nrow(data.frame_AAL)
WEIGHT_AAL <- colSums(V_04, na.rm = TRUE)/nrow(data.frame_AAL)*100
```

Now, we calculate the weights.

```{r}
all_weights <- sum(WEIGHT_AMZN, WEIGHT_ADBE, WEIGHT_MSFT, WEIGHT_AAL)
...
final_weight_AAL <- WEIGHT_AAL/all_weights


all_weights_tb <- tibble(final_weight_AMZN, final_weight_ADBE, final_weight_MSFT, final_weight_AAL)
...
weights_portfolio <- c(0.25, 0.25, 0.25, 0.25,
            weights[1], weights[2], weights[3], weights[4],
             final_weight_AMZN, final_weight_ADBE, final_weight_MSFT, final_weight_AAL)
...
```{r}
mult_stocks <- tq_get(c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT"),get = "stock.prices",from = "2000-01-01", to = "2020-09-17")
mult_Ra <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% 
tq_get(get= "stock.prices", from ="2000-01-01", to = "2020-09-17") %>% 
group_by(symbol) %>% 
tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Mu")
mult_sigma <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% 
tq_get(get= "stock.prices", from ="2000-01-01", to = "2020-09-17") %>% 
group_by(symbol) %>% 
...
mult_period <- tq_get(c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT"),get = "stock.prices",from = "2005-01-01", to = "2020-09-17")
mult_Ra_period <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2005-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Mu")
mult_sigma_period <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2005-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Sigma")
...
mult_period_nd <- tq_get(c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT"),get = "stock.prices",from = "2010-01-01", to = "2020-09-17")
mult_Ra_period_nd <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2010-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Mu")
mult_sigma_period_nd <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2010-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Sigma")
...
mult_period_rd <- tq_get(c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT"),get = "stock.prices",from = "2015-01-01", to = "2020-09-17")
mult_Ra_period_rd <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2015-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Mu")
mult_sigma_period_rd <- c("AMZN","ADBE","MSFT","AAL","PG","AXP","BAC","AAPL","T","CAT") %>% tq_get(get= "stock.prices", from ="2015-01-01", to = "2020-09-17") %>% group_by(symbol) %>% tq_transmute(select = adjusted, mutate_fun = periodReturn, period ="yearly",col_rename = "Sigma")
...
First, we want rolling regressions. We downlaod the S&P500 with prices of stock that have data back to 2000-01-01. 
...
Then, we select our 4 stocks.

```{r}
SP500_2<-c("AMZN", "ADBE", "MSFT", "AAL") %>% tq_get(get  = "stock.prices",from = "2000-12-25", to   = "2018-12-31")

SP500_2000<-slice(SP500_2,1:10)
```

Next, we download fama-french. 

```{r}
factors_data_address <- "http://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/Global_3_Factors_CSV.zip"
factors_csv_name <- "Global_3_Factors.csv"
temp <- tempfile()

download.file(
  # this is the location of the file to be downloaded
  factors_data_address,
  # here we want to store the file
  temp, 
  quiet = TRUE)
#Now, we want to read the file and get the data from 2000-01-01 to 2018-12-31
factors <- 
...
Now, we want the monthly stock returns.
...
fama_french
# Now, we create a function in order to deliver the monthly returns since 2000
...
#Now, we want the returns for our four stocks
amzn_return <- get_monthly_returns("AMZN")
adbe_return <- get_monthly_returns("ADBE")
msft_return <- get_monthly_returns("MSFT")
aal_return <- get_monthly_returns("AAL")
```

Next, we want to merge the two datasets - the stock returns and fama_french

```{r}
joined_data_amzn <- left_join(amzn_return, fama_french, by = NULL)
joined_data_amzn
joined_data_adbe <- left_join(adbe_return, fama_french, by = NULL)
joined_data_adbe
joined_data_msft <- left_join(msft_return, fama_french, by = NULL)
joined_data_msft 
joined_data_aal <- left_join(aal_return, fama_french, by = NULL)
joined_data_aal
```

In a next step, we calculate the reutrns minus the risk free rate. We combine monthly stock returns and risk factors. 

```{r}
joined_data_amzn <- mutate(joined_data_amzn, monthly_ret_rf = monthly_return - rf)
joined_data_adbe <- mutate(joined_data_adbe, monthly_ret_rf = monthly_return - rf)
joined_data_msft <- mutate(joined_data_msft, monthly_ret_rf = monthly_return - rf)
joined_data_aal <- mutate(joined_data_aal, monthly_ret_rf = monthly_return - rf)
```

#rolling CAPM of our 4 stocks

In the next part, we want to create rolling CAPM of our4 stocks. We add the rolling CAPM to the Tibble ofr each Stock. 
...
joined_data_amzn <- add_rolling_capm(joined_data_amzn)
joined_data_adbe <- add_rolling_capm(joined_data_adbe)
joined_data_msft <- add_rolling_capm(joined_data_msft)
joined_data_aal <- add_rolling_capm(joined_data_aal)

```

#plotting ALPHA

In this part, we want to create plots for alpha.
...
create_alpha_plot(joined_data_amzn, "Amazon: 5 year rolling CAPM - alpha")
create_alpha_plot(joined_data_adbe, "Adobe: 5 year rolling CAPM - alpha")
create_alpha_plot(joined_data_msft, "Microsoft: 5 year rolling CAPM - alpha")
create_alpha_plot(joined_data_aal, "American Airlines: 5 year rolling CAPM - alpha")
```

#plotting beta

In this part, we want to create plots for beta. 
...
create_beta_plot(joined_data_amzn, "Amazon: 5 year rolling CAPM - beta")
create_beta_plot(joined_data_adbe, "Adobe: 5 year rolling CAPM - beta")
create_beta_plot(joined_data_msft, "Microsoft: 5 year rolling CAPM - beta")
create_beta_plot(joined_data_aal, "American Airlines: 5 year rolling CAPM - beta")
```

#plotting all four CAPM alpha

Now, we want to plot all four CAPM alpha's. 
 
```{r}

joined_data <- rbind(
  add_column(select(joined_data_amzn, alpha, beta, date), "symbol" = "AMZN"),
  add_column(select(joined_data_adbe, alpha, beta, date), "symbol" = "ADBE"),
  add_column(select(joined_data_msft, alpha, beta, date), "symbol" = "MSFT"),
  add_column(select(joined_data_aal, alpha, beta, date), "symbol" = "AAL")
)
...
#plotting all four CAPM beta

Now, we want to plot all four CAPM beta's. 
