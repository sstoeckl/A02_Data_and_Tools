...
#Read the file and get the data between 2000-01-01 and 2019-12-31
factors <- 
...
load(tempf2)
ff<-FFdownload$x_Developed_3_Factors$monthly
sff<-as.data.frame(ff)
sff<-rownames_to_column(sff)
colnames(sff)[1]<-"Date"
sff<-rename(t,c("Temp2.Mkt.RF" = "Mkt.RF"))
sff<-rename(t,c("Temp2.SMB" = "SMB"))
sff<-rename(t,c("Temp2.HML" = "HML"))
sff<-rename(t,c("Temp2.RF" = "RF"))
sff
```

to continue here with the download from FF, simply exchange "fama_french" with "sff" in the following chunk. The rest can stay the same.
...
fama_french <- filter(factors, date >= as.Date("2000-01-01"), date <= as.Date("2019-12-31"))
...
joined_data_msft <- merge(msft_return, fama_french, by="date")
joined_data_msft

joined_data_qcom <- merge(qcom_return, fama_french, by ="date")
joined_data_qcom

joined_data_axp <- merge(axp_return, fama_french, by = "date")
joined_data_axp 

joined_data_gild <- merge(gild_return, fama_french, by = "date")
joined_data_gild
```
