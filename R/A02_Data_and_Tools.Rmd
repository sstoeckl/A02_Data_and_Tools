...
# b) Download stock prices of stocks that had information at the 25.Dec.2000. 
#    Slice them to only select 10!
SP500_2000<-c("MSFT", "QCOM", "GOOG", "GILD") %>% tq_get(get  = "stock.prices",from = "2000-12-25", to   = "2019-12-31")
...
goog_return <- get_monthly_returns("GOOG")
...
joined_data_goog <- left_join(goog_return, fama_french, by = NULL)
joined_data_goog 
...
joined_data_goog <- mutate(joined_data_goog, monthly_ret_rf = monthly_return - rf)
...
joined_data_goog <- add_rolling_capm(joined_data_goog)
...
create_alpha_plot(joined_data_goog, "Google: 5 year rolling CAPM - alpha")
...
create_beta_plot(joined_data_goog, "Google: 5 year rolling CAPM - beta")
...
joined_data <- rbind(
  ...
  add_column(select(joined_data_goog, alpha, beta, date), "symbol" = "GOOG"),
